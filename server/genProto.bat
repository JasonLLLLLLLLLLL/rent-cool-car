...
%PBTS_BIN_DIR%\pbjs -t static -w es6 trip.proto --no-create --no-encode --no-decode --no-verify --no-delimited -o %PBTS_OUT_DIR%\trip_pb_tmp.js && %PBTS_BIN_DIR%\pbts -o %PBTS_OUT_DIR%\trip_pb.d.ts %PBTS_OUT_DIR%\trip_pb.js

echo 'import * as $protobuf from "protobufjs";\n' > %PBTS_OUT_DIR%\trip_pb.js
cat %PBTS_OUT_DIR%\trip_pb——tmp.js >>  %PBTS_OUT_DIR%\trip_pb.js